...
public class MainPanel extends JPanel implements ActionListener, ListSelectionListener{
	...
	JButton clearusers;
	JButton parameters;
	JOptionPane cancelPane = new JOptionPane();
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource() == enroll){
			if (engine.getUsers().size() >= engine.getMaxUserCount()){
				SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this, "Maximum number of users is reached.", "Failed", JOptionPane.ERROR_MESSAGE); });
				return;
			}
			...
			try {
				EngineEnroll engineEnroll = new EngineEnroll();
				engineEnroll.execute();
				...
				if(engine.getEngineStatus() != NffvStatus.TemplateCreated){
					SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this, "Enroll failed - " + engine.getEngineStatus(), "Failed", JOptionPane.ERROR_MESSAGE); });
					return;
				}
			
				ImageIcon imageIcon = curruser.getNffvImage().getImageIcon();
				img.setIcon(imageIcon);
				img.setBorder(BorderFactory.createEmptyBorder(0, ((getWidth() - users.getWidth()) / 2) - (imageIcon.getIconWidth() / 2), 0, 0));
				usersmodel.addElement(new IDName(curruser.getID(),name));
				users.setSelectedIndex(usersmodel.size() - 1);
				saveIDNames();
			}catch (Exception e) {
				e.printStackTrace();
				SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this, "Enroll failed - " + e.getMessage(), "Failed", JOptionPane.ERROR_MESSAGE); });
			}
		}
		...
		if(arg0.getSource() == verify){
			...
			if(selection == null){
				SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this,"Please select user to verify with","Error",JOptionPane.ERROR_MESSAGE); });
				return;
			}
			NffvUser user = engine.getUserByID(selection.getID());
			EngineVerify engineVerify = new EngineVerify(user);
			engineVerify.execute();
			...
			if (engine.getEngineStatus() == NffvStatus.TemplateCreated){
				if( score > 0) 
					SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this,selection.getName() + " verified. \n Fingerprint matching score: " + score,"Verification",JOptionPane.DEFAULT_OPTION); });
				else 
					SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this,selection.getName() + " not verified.\nFingerprints did not match.","Verification failed",JOptionPane.ERROR_MESSAGE); });
			}
			else{
				SwingUtilities.invokeLater(() -> { JOptionPane.showMessageDialog(this,"Verification failed - " + engine.getEngineStatus(),"Failed",JOptionPane.ERROR_MESSAGE); });
			}
		}
	...
	private boolean cancelPane() {
		int cancel = JOptionPane.showOptionDialog(null, "Wating for fingerprint...", "",
				JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, new Object[]{"Cancel"}, "Cancel");
		if (cancel == 0) {
			return true;
		}
		else {
	...
	class EngineEnroll extends SwingWorker<NffvUser, Object> {

		@Override
		public NffvUser doInBackground() {
		...
		public void cancel() {
			engine.cancel();
			do {
				JOptionPane.showOptionDialog(null, "Canceling...","", JOptionPane.DEFAULT_OPTION,
	...
	class EngineVerify extends SwingWorker<Integer, Object> {

		NffvUser user;
		EngineVerify(NffvUser user) {
			this.user = user;
		}

		@Override
		public Integer doInBackground() {
			Integer score = engine.verify(user, TIMEOUT);
			return score;
		}
